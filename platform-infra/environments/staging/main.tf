...
module "vpc" {
  ...
  availability_zones = slice(data.aws_availability_zones.available.names, 0, 2) # Staging mirrors prod with 2 AZs for HA
...
module "s3_media_bucket" {
  ...
  bucket_name = "${var.project_name}-media-staging"
  versioning  = true
  public_read = true # Staging can be public for testing, but proxied by Cloudflare
  tags = {
...
module "rds" {
  ...
  name                   = "${var.project_name}-db-staging"
...
module "elasticache" {
  ...
  parameter_group_name     = "default.redis7" # A custom PG could be used
...
module "opensearch" {
  ...
  domain_name    = "${var.project_name}-search-staging"
...
module "cloudflare" {
  ...
  app_load_balancer_dns = "staging-placeholder-alb.us-east-1.elb.amazonaws.com" # This would be an output from an ALB
}
